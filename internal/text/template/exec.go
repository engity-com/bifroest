...
import (
	"errors"
	"fmt"
	"github.com/engity-com/bifroest/internal/fmtsort"
	"io"
	"reflect"
	"runtime"
	"strings"
...
func (s *state) evalField(dot reflect.Value, fieldName string, node parse.Node, args []parse.Node, final, receiver reflect.Value) reflect.Value {
	...
	if res, ok := s.tryEvalAsGetter(dot, fieldName, node, ptr); ok {
		return res
	}
...
func (s *state) printValue(n parse.Node, v reflect.Value) {
	s.at(n)
	iface, ok := printableValue(v, s.tmpl.option.onInvalid)
	if !ok {
		s.errorf("can't print %s of type %s", n, v.Type())
	}
	if iface == nil {
		switch s.tmpl.option.onNil {
		case nilPrintNil:
			iface = "<nil>"
		case nilPrintEmpty:
			iface = ""
...
func printableValue(v reflect.Value, ia invalidAction) (any, bool) {
