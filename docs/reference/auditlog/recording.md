...
At startup, Bifröst completes interrupted publication and verifies active recordings before opening listeners. Recoverable physical tails are truncated to the last durable boundary, and interrupted active recordings are sealed as `incomplete` with reason `startup-recovery`. Startup recovery does not synthesize a final recording lifecycle audit event; a started event without a final event can therefore indicate a process interruption or a final audit-write failure.
