...
For least-privilege offline verification, `audit export` and `audit verify` accept `--source PATH --expectedProducerId ID` without a configuration; `audit decrypt` is an alias of export. The producer ID is the 64-hex-character SHA-256 identity of the signing public key. Record it through an independently trusted provisioning channel; do not take it from the journal currently being verified or extract it automatically from a container. An optional auditlog name labels the offline source and defaults to `default`. When a source has an explicit expected producer ID, its configured signing private key is not opened and may be absent. Without this flag, configured sources use their signing identity; offline export and verify require an external trust anchor.

On the Bifröst host, [`bifroest audit producer-id`](producer-id.md) prints this ID from the configured signing private key without reading the journal. Preserve that value through a trusted channel before using it as an offline trust anchor. Locally, `audit verify default` and `audit export default` already load the platform's default configuration and use its signing identity; neither `--configuration` nor `--expectedProducerId` is needed unless selecting a different configuration or working without the signing key.
...
Commands select auditlog **names**, not segment paths. For offline export or verify, copy the complete stopped local journal root, including `<root>/<producer-id>/head.cbor`, all sealed segments, and any active segment. Supply the copy as `--source` with an independently trusted `--expectedProducerId` instead of creating an offline configuration. The signed head is required. The server's signing private key must not be copied; keep any age recipient private key solely on the offline workstation. `audit merge` uses configured sources. See the [encrypted audit example](../../auditlog/index.md#encrypted-audit-events).
...
* [`bifroest audit verify`](verify.md) verifies a journal without exporting records and reports whether verification was outer or full.
