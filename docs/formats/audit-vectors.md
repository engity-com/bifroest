...
These are **audit journal**, not session [recording](recording.md), vectors.
...
For the byte layout and field keys, see the [audit format](audit.md) and
[shared container rules](container.md). Operational CLI verification requires
a configured journal root with a signed head, **not** a bare downloaded
segment; see the [audit log reference](../reference/auditlog/index.md).

## Downloads

The [manifest](../assets/audit-format-vectors/v1/manifest.json) is the strict
version 1 inventory: public test seeds, producer and recipient identity,
sizes, raw file SHA-256 values, reproducibility, and audit domain hashes.
...
These hashes use the [audit-specific domain prefixes](audit.md#audit-identity-and-chain),
...
To update clear vectors and re-extract encrypted units **without changing the
four frozen Age files**, run:
...
The age flag alone is rejected. Review every resulting binary and manifest
change before publishing. Never compare a freshly encrypted segment byte for
byte with the frozen one. A valid self-signature does not establish trust:
pin the expected producer ID and, for encrypted journals, the expected
recipient fingerprint from a trusted source.

The opt-in generator writes multiple files and is **not transactional**. If
it fails, resolve the cause, regenerate, and pass the complete vector test
