...
func TestInt64(t *testing.T) {
	cases := []struct {
		...
		plain:             "{{.foo}}",
		expectedRenderErr: errors.New(`template: int64:1:2: executing "int64" at <.foo>: map has no entry for key "foo"`),
	}, {
		data:              map[string]any{"foo": map[string]any{"bar": "abc"}},
		plain:             "{{.foo.bars}}",
		expectedRenderErr: errors.New(`template: int64:1:6: executing "int64" at <.foo.bars>: map has no entry for key "bars"`),
	}, {
	...
	for i, c := range cases {
		t.Run(fmt.Sprintf("case-%d", i), func(t *testing.T) {
			instance, actualErr := NewInt64(c.plain)
