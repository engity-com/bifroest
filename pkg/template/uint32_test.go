...
func TestUint32(t *testing.T) {
	cases := []struct {
		...
		plain:             "{{.foo}}",
		expectedRenderErr: errors.New(`templated uint32 results in a value that cannot be parsed as uint32: "-11"`),
	}, {
		data:              map[string]any{"foobar": "666"},
		plain:             "{{.foo}}",
		expectedRenderErr: errors.New(`template: uint32:1:2: executing "uint32" at <.foo>: map has no entry for key "foo"`),
	}, {
		data:              map[string]any{"foo": map[string]any{"bar": "abc"}},
		plain:             "{{.foo.bars}}",
		expectedRenderErr: errors.New(`template: uint32:1:6: executing "uint32" at <.foo.bars>: map has no entry for key "bars"`),
	}, {
	...
	for i, c := range cases {
		t.Run(fmt.Sprintf("case-%d", i), func(t *testing.T) {
			instance, actualErr := NewUint32(c.plain)
