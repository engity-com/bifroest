...
func padCastForSha256Checkpoint(writer *CastWriter) (castSha256Checkpoint, error) {
	if writer == nil || writer.digest == nil {
		return castSha256Checkpoint{}, errors.System.Newf("nil cast writer")
	}
	if writer.poisoned != nil {
		return castSha256Checkpoint{}, writer.poisoned
	}
	if writer.sealed {
		return castSha256Checkpoint{}, errors.System.Newf("cast is already sealed")
	}
...
func checkpointCastSha256(hasher hash.Hash) (castSha256Checkpoint, error) {
	...
	if processed%sha256.BlockSize != 0 {
		return castSha256Checkpoint{}, errors.System.Newf("cast SHA-256 checkpoint is not block-aligned")
	}
	if !bytes.Equal(state[4+sha256.Size:4+sha256.Size+sha256.BlockSize], make([]byte, sha256.BlockSize)) {
		return castSha256Checkpoint{}, errors.System.Newf("cast SHA-256 checkpoint contains buffered input")
	}
...
func resumeCastSha256(checkpoint castSha256Checkpoint) (hash.Hash, error) {
	if checkpoint.Bytes == 0 || checkpoint.Bytes%sha256.BlockSize != 0 {
		return nil, errors.System.Newf("illegal Cast SHA-256 checkpoint byte count %d", checkpoint.Bytes)
	}
	...
	if !ok {
		return nil, errors.System.Newf("SHA-256 implementation cannot restore state")
	}
	if err := unmarshaler.UnmarshalBinary(state); err != nil {
		return nil, errors.System.Newf("cannot restore Cast SHA-256 checkpoint: %w", err)
	}
	return hasher, nil
}
...
func marshalCastSha256(hasher hash.Hash) ([]byte, uint64, error) {
	if hasher == nil || hasher.Size() != sha256.Size || hasher.BlockSize() != sha256.BlockSize {
		return nil, 0, errors.System.Newf("illegal Cast SHA-256 implementation")
	}
	marshaler, ok := hasher.(encoding.BinaryMarshaler)
	if !ok {
		return nil, 0, errors.System.Newf("SHA-256 implementation cannot expose checkpoint state")
	}
	state, err := marshaler.MarshalBinary()
	if err != nil {
		return nil, 0, errors.System.Newf("cannot read Cast SHA-256 state: %w", err)
	}
	if len(state) != sha256MarshaledSize || !bytes.Equal(state[:4], sha256MarshaledMagic[:]) {
		return nil, 0, errors.System.Newf("unsupported SHA-256 checkpoint encoding")
	}
